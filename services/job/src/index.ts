  ...
  try {
    await sql`
        ...
        BEGIN
            ...
            THEN CREATE TYPE job_type AS ENUM ('Full-time', 'Part-time', 'Contract', 'Internship');
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'work_location') 
            THEN CREATE TYPE work_location AS ENUM ('Remote', 'On-site', 'Hybrid');
            END IF;
...
`;
    ...
    console.log("Job Service Database checked and created Successfully");
  } catch (error) {
