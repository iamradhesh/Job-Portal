...
router.post("/upload", async (req: Request, res: Response) => {
  try {
    const { buffer, originalname, mimetype, public_id } = req.body as UploadRequestBody;
    ...
    cleanPublicId = cleanPublicId.replace("job-portal/job-portal/", "job-portal/");
    ...
    const uploadResult = await cloudinary.uploader.upload(uploadDataURI, uploadOptions);
